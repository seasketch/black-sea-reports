...
export async function humanUseAreaOverlap(
  sketch: Sketch<Polygon> | SketchCollection<Polygon>
): Promise<ReportResult> {
  const box = sketch.bbox || bbox(sketch);
  const metricGroup = project.getMetricGroup("humanUseAreaOverlap");
...
export default new GeoprocessingHandler(humanUseAreaOverlap, {
  title: "humanUseAreaOverlap",
  description: "Calculate sketch overlap with humanUseology polygons",
  executionMode: "async",
  timeout: 600,
  requiresProperties: [],
});
