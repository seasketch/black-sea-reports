...
export async function seabedAreaOverlap(
  sketch: Sketch<Polygon> | SketchCollection<Polygon>
): Promise<ReportResult> {
  const box = sketch.bbox || bbox(sketch);
  const metricGroup = project.getMetricGroup("seabedAreaOverlap");
...
export default new GeoprocessingHandler(seabedAreaOverlap, {
  title: "seabedAreaOverlap",
  description: "Calculate sketch overlap with seabedology polygons",
  executionMode: "async",
  timeout: 600,
  requiresProperties: [],
});
